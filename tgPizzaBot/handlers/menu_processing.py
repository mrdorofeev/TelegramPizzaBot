...
async def products(session, level, category, page):
    current_products = await orm_get_products(session, category_id=category)
    paginator = Paginator(current_products, page=page)
...
async def carts(session, level, menu_name, page, user_id, product_id):
    ...
    current_carts = await orm_get_user_carts(session, user_id)
    ...
    if not current_carts:
        ...
        paginator = Paginator(current_carts, page=page)
        cart = paginator.get_page()[0]
        cart_price = round(cart.quantity * cart.product.price, 2)
        total_price = round(sum(cart.quantity * cart.product.price for cart in current_carts), 2)
        image = InputMediaPhoto(
            media=cart.product.image,
            caption=f"<strong>{cart.product.name}\n\
                    ${cart.product.price} * {cart.quantity} = ${cart_price}\n\
                    Товар {paginator.page} из {paginator.pages} в корзине</strong>\n\
                    Итого: ${total_price}",
        )
